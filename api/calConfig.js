...
const CAL_API_URL = "https://api.calendly.com";
const CAL_API_KEY = process.env.CAL_API_KEY;

// Fetch Calendly user details
export const getCalendlyUser = async () => {
  try {
    const response = await axios.get(`${CAL_API_URL}/users/me`, {
      headers: {
...
export const setCalendlyAvailability = async (
  schedulingUrl,
  startDate,
  endDate
) => {
...
export const listEventTypes = async (userUri) => {
  try {
    const response = await axios.get(`${CAL_API_URL}/event_types`, {
      headers: {
        Authorization: `Bearer ${CAL_API_KEY}`,
        "Content-Type": "application/json",
      },
